...
contract SubscriptionsController is HyperApp, Ownable, ReentrancyGuard {
    ...
    event DispatchFailed(uint256 indexed chainId, address indexed adapter, uint8 messageType);
    event DispatchTimeout(uint8 messageType);
    ...
    function onAccept(
        ...
        require(chainRegistry.isValidAdapter(_chainid, adapter), 'UNREGISTERED_ADAPTER');
    ...
    function onPostRequestTimeout(
        ...
        if (_type == 0) {
            // Token update timed out - clear sync
            bytes32 syncId = keccak256(_params);
            if (tokenSyncs[syncId]) {
                tokenSyncs[syncId] = false;
            }
        } else if (_type == 2) {
            // Merchant update timed out - clear sync
            bytes32 syncId = keccak256(_params);
            if (merchantSyncs[syncId]) {
                merchantSyncs[syncId] = false;
            }
        }
    ...
    function relayChargeRequest(
        ...
        try IDispatcher(_host).dispatch(post) {
            ...
            emit DispatchFailed(_chain, _adapter, 1);
        }
    }
    ...
    function relayMerchantProfileUpdate(
        ...
        try IDispatcher(_host).dispatch(post) {
            ...
            emit DispatchFailed(_chain, _adapter, 2);
        }
    }
}
