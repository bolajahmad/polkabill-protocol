...
async function processLog(log: any, store: EntityManager) {
  if (log.topics[0] == merchantRegAbi.events.MerchantUpdated.topic) {
    ...
    console.log("Charge Request Relayed event.");
    handleChargeRequestRelayed(log, store)
    console.log("Is charge stored: ", store.charges.values());
    ...
    console.log("Charge Confirmed event. Is charge stored: ", store.charges.values());
  } else {
    return;
  }
}
