...
export const UpdateTokenModal = ({ chainId, onComplete }: Props) => {
  ...
  const { mutate: updateAdapterTokens, isPending } = useWriteContract({
    mutation: {
      onError: error => {
  ...
  return (
    <div>
      <div className="space-y-4">
        <div className="p-4 bg-emerald-50 rounded-xl border border-emerald-100 flex gap-3 mb-4">
          ...
          <p className="text-xs text-emerald-700 leading-relaxed">
            Chain registered successfully! Now add the stablecoin addresses supported on this
            network.
          </p>
        </div>
        <div className="space-y-2">
          ...
          />
          {!!token && !validToken && <FieldError errors={[{ message: 'Invalid token address' }]} />}
        </div>
      ...
      <div className="w-full">
        ...
        >
          {isPending ? <Loader2 className="animate-spin" size={16} /> : 'Add Token'}
        </Button>
      </div>
    </div>
  );
